...
#define DMT_STACK_TRACE_MAX 320
#endif
#endif
...
void *_dmt_alloc(size_t sz, int zeroset) {
  dmt_node_t *node = NULL;
  
  if (zeroset) {
    node = __real_calloc(sizeof(*node) + sz, 1);
  } else {
    node = __real_malloc(sizeof(*node) + sz);
    if (node != NULL) {
...
void *_dmt_realloc(void *ptr, size_t sz) {
  dmt_node_t *node = (dmt_node_t*)((char*)ptr - sizeof(*node));
  dmt_node_t *old_node = node;

  if (ptr == NULL) return _dmt_alloc(sz, 0);

#ifndef DMT_UNSAFE
  if (!_dmt_has_node(node)) {
    fprintf(stderr, "Bad realloc: %p \n", ptr );
    _dmt_abort();
  }
#endif

  node = __real_realloc(node, sizeof(*node) + sz);
...
void * __wrap_malloc( size_t x ){ 
     return _dmt_alloc(x, 0 );
    } 



void *  __wrap_calloc( int num, size_t sz) {
     return   _dmt_alloc((num) * (sz), 1);   
     } 
