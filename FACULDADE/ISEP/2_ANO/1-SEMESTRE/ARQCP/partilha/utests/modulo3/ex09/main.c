...
int* call_func (int* (*f)(int* ptr, int num, int x), int* ptr , int num, int x );  
...
void run_test(int * vec, int in_num, int to_search, int expected_pos )
{
    int vec1[100];
	...
	res = call_func(vec_search,vec1+1,in_num,to_search);
...
void test_One()
{ 
    run_test((int[]){1},1,1,0); 
}
void test_Zero()
{ 
    run_test((int[]){1,0,-1},3,0,1); 
}
void test_Minus3()
{ 
    run_test((int[]){-1,-1,-1,-3},4,-3,3); 
}
void test_Five()
{ 
    run_test((int[]){-1,-1,-1,-3,1},5,1,4); 
}
void test_NotFound()
{ 
    run_test((int[]){-1,-1,-1,-3,1},5,0,-1); 
}
